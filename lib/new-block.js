...
function getInboundTxs(cryptoSymbol, bestBlockHash, err, objectNewTransactions, stuff1, stuff2) {
...
//    console.log(logPrefix + "getInboundTransactions: objectNewTransactions: " + objectNewTransactions[0],toString());
    ...
    var x = recordInboundTxs(inboundTransactions, cryptoSymbol, bestBlockHash) // remove last 2 vars when you fix the obj
}
...
function something(bcAccount, objectDeposit, cryptoSymbol, bestBlockHash){
...
function recordInboundTxs(txList, cryptoSymbol, bestBlockHash) {
    ...
    for (each in txList){
        ...
        something(txList[each].address, deposit, coinSymbol, bestBlockHash); //iteration while querying worked out REALLY badly so this function keeps data coherent.
    }
}


function main(cryptoSymbol, bestBlockHash, coinDaemonPool, database, logger){
//cli for testing only
