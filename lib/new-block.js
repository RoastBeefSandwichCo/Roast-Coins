...
function instance_locked (){
    fs.access('.lock', fs.F_OK, function(err) {  // one instance!
        ...
        new_lock = fs.writeFileSync('.lock', 'one instance!\n');
...
function recordInboundTxs(txList) {
    ...
    var arrayWriteTheseTxs = []  // array of deposits to record
    for (each in txList){
        ...
        .then(function(data){
            if (data.length == 0) {  //empty response! #TODO: something more constructive here.
                ...
                arrayPromises.push(database.recordTransaction(deposit)); //all writes must finish if this module is to closeDb
            }
        });
